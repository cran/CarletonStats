...
groupedBar.default <-
  function(
    ...
    if (!requireNamespace("ggplot2", quietly = TRUE)) {
      stop("ggplot2 packaged needed for this function to work.")
    }
    ...
    if (is.null(condvar)) {
      ...
      p <- ggplot2::ggplot(out.df) +
        ggplot2::geom_bar(
          ggplot2::aes(x = .data[["resp"]], y = .data[["Freq"]]),
          stat = "identity",
          fill = "steelblue"
        ) +
      ...
      if (percent) {
        ...
        temp4 <- cbind(temp2, rowSums(temp2))
        dimnames(temp4)[[2]][m + 1] <- "Sum"
      }
    } #end else
