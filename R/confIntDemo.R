...
confIntDemo <-
  function(distr = "normal", size = 20, conf.level = .95) {
    if (size > 100000) {
      stop("Choose smaller sample size")
    }
    if (size < 2) {
      stop("Choose larger sample size")
    }

    alpha <- 1 - conf.level
    conf.text <- deparse(substitute(conf.level))
    title.text <- paste(
      "100 confidence intervals, confidence level",
      conf.text,
      sep = " "
    )
    ...
    popdist <- pmatch(
      distr,
      c("binary", "normal", "exponential", "uniform"),
      nomatch = NA
    )
    if (is.na(distr)) {
      stop(
        "Distribution must be one of \"binary\", \"normal\", \"exponential\" or \"uniform\""
      )
    }
    ...
    for (j in 1:100) {
      lines(
        temp[j, ],
        c(j, j),
        col = ifelse(index[j], 1, 2),
        lwd = ifelse(index[j], 1, 2)
      )
    }
