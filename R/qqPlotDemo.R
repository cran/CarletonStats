...
qqPlotDemo <-
  function(
    ...
    distr <- pmatch(
      distribution,
      c("normal", "t", "exponential", "chi.square", "F", "beta"),
      nomatch = NA
    )
    if (is.na(distr)) {
      stop(
        "Distribution must be one of \"normal\", \"t\", \"exponential\", \"chi.square\", \"F\", or \"beta\" "
      )
    }
    ...
    distr.expand <- char.expand(
      distribution,
      c("normal", "t", "exponential", "chi.square", "F", "beta"),
      nomatch = warning("No match")
    )
